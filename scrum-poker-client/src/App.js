...
function App() {
  ...
  function Content() {
    ...
    if (createRoom) {
      return (
        <RoomCreateSection onRoomName={setNewRoomName}/>
      );
    } else if (!userName || !roomName) {
      console.log("???");
    return (
      <LoginSection onLogin={setUserName} onRoomName={setRoomName} />
    );
    } else {
      return (
        <div>
  ...
  function LoginSection({ onLogin, onRoomName}) {
    ...
    return (
      <div>
        Username: <input name="username" placeholder="User name" onInput={(e) => setUsername(e.target.value)} />
        <br/>
        Room id: <input
          type="number"
          placeholder="Room id"
